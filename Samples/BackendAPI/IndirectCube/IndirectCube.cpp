...
BIGOS::RESULT IndirectCube::OnInit()
{
...
void IndirectCube::OnUpdate()
{
    m_constantBufferData.model = glm::rotate( glm::mat4( 1.0f ), glm::radians( 90.0f ), glm::vec3( 1.0f, 0.0f, 0.0f ) );
    m_constantBufferData.view  = glm::lookAt( glm::vec3( 2.0f, 2.0f, 2.0f ), glm::vec3( 0.0f, 0.0f, 0.0f ), glm::vec3( 0.0f, 1.0f, 0.0f ) );
...
void IndirectCube::OnRender()
{
    ...
    vbBindDesc.elementStride = 5 * sizeof( float );
    ...
    BIGOS::Driver::Backend::ExecuteIndirectDesc drawDesc;
    drawDesc.hCommandLayout       = m_hCommandLayout;
    drawDesc.hIndirectBuffer      = m_hIndirectBuffer;
    drawDesc.indirectBufferOffset = 0;
    drawDesc.maxCommandCount      = 1;
    drawDesc.hCountBuffer         = ResourceHandle();
    drawDesc.countBufferOffset    = 0;
    m_pCommandBuffers[ bufferNdx ]->ExecuteIndirect( drawDesc );
...
void IndirectCube::OnDestroy()
{
    ...
    m_pAPIDevice->DestroyResource( &m_hIndirectBuffer );
...
BIGOS::RESULT IndirectCube::LoadAssets()
{
    // Load shaders
    if( BGS_FAILED( readDataFromFile( "../Samples/BackendAPI/IndirectCube/shader.hlsl", &m_pShaderSource, &m_shaderSourceSize ) ) )
    {
        return BIGOS::Results::FAIL;
    }
...
BIGOS::RESULT IndirectCube::CreateApplicationResources()
{
    ...
    BIGOS::Driver::Backend::ResourceDesc indDesc;
    indDesc.size.width      = sizeof( INDIRECT_DATA );
    indDesc.size.height     = 1;
    indDesc.size.depth      = 1;
    indDesc.mipLevelCount   = 1;
    indDesc.arrayLayerCount = 1;
    indDesc.format          = BIGOS::Driver::Backend::Formats::UNKNOWN;
    indDesc.resourceLayout  = BIGOS::Driver::Backend::ResourceLayouts::LINEAR;
    indDesc.resourceType    = BIGOS::Driver::Backend::ResourceTypes::BUFFER;
    indDesc.sampleCount     = BIGOS::Driver::Backend::SampleCount::COUNT_1;
    indDesc.sharingMode     = BIGOS::Driver::Backend::ResourceSharingModes::EXCLUSIVE_ACCESS;
    indDesc.resourceUsage   = BGS_FLAG( BIGOS::Driver::Backend::ResourceUsageFlagBits::INDIRECT_BUFFER ) |
                            BGS_FLAG( BIGOS::Driver::Backend::ResourceUsageFlagBits::TRANSFER_DST );
    if( BGS_FAILED( m_pAPIDevice->CreateResource( indDesc, &m_hIndirectBuffer ) ) )
    {
        return BIGOS::Results::FAIL;
    }
    ...
    BIGOS::Driver::Backend::ResourceAllocationInfo indAllocInfo;
    m_pAPIDevice->GetResourceAllocationInfo( m_hIndirectBuffer, &indAllocInfo );

    BIGOS::Driver::Backend::AllocateMemoryDesc bufferAllocDesc;
    bufferAllocDesc.size      = vbAllocInfo.size + ibAllocInfo.size + indAllocInfo.size;
    ...
    bindMemDesc.hResource    = m_hIndirectBuffer;
    bindMemDesc.memoryOffset = vbAllocInfo.size + ibAllocInfo.size;
    if( BGS_FAILED( m_pAPIDevice->BindResourceMemory( bindMemDesc ) ) )
    {
        return BIGOS::Results::FAIL;
    }
...
BIGOS::RESULT IndirectCube::UploadResourceData()
{
    BGS_ASSERT( m_pQueue );
    ...
    cpyBuffDesc.hDstBuffer = m_hIndirectBuffer;
    cpyBuffDesc.srcOffset  = sizeof( VERTEX_DATA ) + sizeof( INDEX_DATA );
...
BIGOS::RESULT IndirectCube::CreatePipeline()
{
    // Command layout
    BIGOS::Driver::Backend::CommandLayoutDesc cmdLayoutDesc;
    cmdLayoutDesc.type   = BIGOS::Driver::Backend::IndirectCommandTypes::DRAW_INDEXED;
    cmdLayoutDesc.stride = sizeof( INDIRECT_DATA );
    if( BGS_FAILED( m_pAPIDevice->CreateCommandLayout( cmdLayoutDesc, &m_hCommandLayout ) ) )
    {
        return BIGOS::Results::FAIL;
    }
